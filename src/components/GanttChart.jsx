...
export const GanttChart = ({ data, drillState, onTogglePhase, onToggleMilestone }) => {
    ...
    const getPositionStyles = (startTime, endTime) => {
        const left = ((startTime - minDate) / (1000 * 60 * 60 * 24)) * dayWidth;
        const width = Math.max(((endTime - startTime) / (1000 * 60 * 60 * 24)) * dayWidth, 10);
    ...
    return (
        <div className="flex h-full w-full bg-slate-900 overflow-hidden text-sm">
            ...
            {/* Main Gantt Canvas */}
            <div className="flex-1 overflow-auto bg-slate-950 relative hide-scrollbar">
                                        ...
                                        className={clsx(
                                            ...
                                            item.type === 'phase' ? 'bg-indigo-600/80 border border-indigo-500' :
                                                item.type === 'milestone' ? 'bg-orange-600/80 border border-orange-500 h-5' :
                                                    'bg-cyan-600/60 border border-cyan-500 h-4'
                                        )}
                                        style={getPositionStyles(item.startTime, item.endTime)}
                                    >
                                        {/* Progress Overlay */}
                                        {item.type === 'task' && item.progress && (
                                            <div
                                                className="absolute left-0 top-0 h-full bg-cyan-400/30"
