...
import { FileText, Clock, CheckCircle2, AlertCircle, Users, Download, Maximize, Loader2, BarChart3, Sparkles } from 'lucide-react';
...
export const Dashboard = ({ data }) => {
    ...
    const handleExportGamma = () => {
        setIsExporting(true);
        console.log('Starting Gamma MD Export...');

        try {
            const mdContent = `# Project Health Executive Summary
An automated status debrief generated by Project-Flow.

## 📊 Core Performance Metrics
- **Total Scheduled Tasks:** ${metrics.total}
- **Completion Benchmark:** ${metrics.percentComplete}% successfully resolved
- **Assigned Human Resources:** ${metrics.assigneeCount} active participants
- **Financial Velocity (Earned Value):** $${metrics.totalCost}

## ⚠️ Risk & Timeline Slippage
- **Tasks at Immediate Risk:** ${metrics.atRisk} (Zero remaining baseline buffer)
- **Overdue Action Items:** ${metrics.overdue} (Past absolute baseline deadline)

## 📈 Strategic Guidance
- Maintain close monitoring on the ${metrics.atRisk} tasks exhibiting high constraint.
- Address the ${metrics.overdue} overdue items to recalibrate the Agile Sprint Burndown trajectory.

---
*Generated via Project-Flow Intelligence Engine for Gamma AI integration.*
`;
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = 'Project_Health_Gamma_Export.md';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            URL.revokeObjectURL(url);
            console.log('Gamma MD saved.');
        } catch (error) {
            console.error("Failed to generate Gamma MD:", error);
            alert("Failed to export Gamma AI Markdown: " + String(error.message || error));
        } finally {
            setIsExporting(false);
        }
    };
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="no-export flex items-center gap-2 bg-fuchsia-600 hover:bg-fuchsia-500 text-white px-4 py-2 rounded-lg font-semibold transition-all shadow-lg hover:shadow-fuchsia-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                        title="Download Markdown for Gamma AI"
                    >
                        {isExporting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                        {isExporting ? 'Generating...' : 'Export to Gamma'}
                    </button>
                    <button
                        type="button"
